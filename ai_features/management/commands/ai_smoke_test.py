...
class Command(BaseCommand):
    help = "Diagnostic smoke test for SkyVault AI infrastructure (PostgreSQL pgvector, generation endpoints, local embeddings)."
    ...
    def handle(self, *args, **options):
        ...
        # 4. Test embeddings integration (sentence-transformers)
        self.stdout.write("\n4. Testing Embeddings Integration (sentence-transformers)...")
        emb_model = getattr(settings, "AI_EMBEDDING_MODEL", "all-MiniLM-L6-v2")
        self.stdout.write(f"   Embedding Model: {emb_model} (384 dimensions)")
        self.stdout.write("   Generating test embedding vector...")
        vec = embed_text("SkyVault cloud file management system")
        if vec and len(vec) == 384:
            self.stdout.write(self.style.SUCCESS(f"   [SUCCESS] Received 384-dim embedding vector! Sample: {vec[:3]}..."))
        else:
